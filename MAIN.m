...
%% --- load and analyze grass site sonic data
...
%% --- load and analyze SLTEST site sonic data
...
for  I = 1:3
    ...
    [Up(I),Tp(I)] = process_data_2(data(I).U,data(I).T,pow,fs,data(I).ustar,z_vec);  
end
...
for I = 1:3 % steps through each stability
    ...
    for k = 1:size(Us,3)
        ...
        p = polyfit(log(z),U,1);
        a = p(1);
        dUdz(:,k) = a./z_vec;   % shear at each height 
        dUdz_2m(k) = a./2;
        plot(U,z,'ko'); hold on
    ...
    for k = 1:size(Ts,3)
        ...
        p = polyfit(log(z),T,1);
        a = p(1);
        dTdz(:,k) = a./z_vec;   % shear at each height  
        dTdz_2m(k) = a./2;
        plot(T,z,'ko'); hold on
        zk = linspace(z_vec(1),z_vec(end),20); 
        Tk = polyval(p,log(zk)); 
